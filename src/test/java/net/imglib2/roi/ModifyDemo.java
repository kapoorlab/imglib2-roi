...
import net.imglib2.roi.Bounds.SmallestContainingInterval;
...
//		final RealMaskRealInterval composite = s1.xor( s2 ).xor( s3 );
		...
		final RealRandomAccessibleRealInterval< BoolType > s1rrai = Masks.toRealRandomAccessibleRealInterval( s1 );
		...
		final RealRandomAccessibleRealInterval< BoolType > s2rrai = Masks.toRealRandomAccessibleRealInterval( s2 );
		...
		final RealRandomAccessibleRealInterval< BoolType > s3rrai = Masks.toRealRandomAccessibleRealInterval( s3 );
		final BdvSource bdv3 = BdvFunctions.show(
				s3rrai,
				new SmallestContainingInterval( s3rrai ),
				"Sphere 3",
				Bdv.options().addTo( bdv1 ).axisOrder( AxisOrder.XYZ ) );
		final RealRandomAccessibleRealInterval< BoolType > compositerrai = Masks.toRealRandomAccessibleRealInterval( composite );
		...
		behaviours.behaviour( new DragBehaviour()
		{
			...
			public void init( final int x, final int y )
			{
				...
				if ( s != null )
				{
					...
					s.center().localize( t );
					LinAlgHelpers.subtract( t, pos, offset );
				}
			}
